...
const AGENT_VERSION = 'agente-aprovacao-v3.2.1';
...
// lead_objections.decision_id antes de consultar autoridade. Enquanto o proprio
// agente-objecoes nao expuser uma rota de execucao aprovada, falha fechado em
// executor_indisponivel, preservando a aprovacao humana separada da execucao.
...
async function executarAcaoAprovada(aprovacao: any): Promise<AcaoExecResult> {
  ...
  if (ehObjecaoTask) {
    ...
    return { ok: false, resultado: 'executor_indisponivel:agente-objecoes_sem_rota_execucao_aprovada', chegou_executor: false, bloqueado: true, identidade: identidade?.motivo };
  }
