...
const CAND = fs.readFileSync(new URL('../candidato.ts', import.meta.url), 'utf8');
...
// ── T5 — ENVIO EXPLICITO + CEP JA DISPONIVEL ──────────────────────────────
{
  ...
  check('T5.b CEP conhecido reaproveitado', e.cep_conhecido === '30140071' && e.cep_fonte === 'slot', JSON.stringify(e));
...
// ── INVARIANTES ESTRUTURAIS: os tres sitios de aplicacao existem no candidato ──
{
  ...
  check('E12 versao logica declarada v4.34.0',
    CAND.includes("const V = 'agente-noturno-v4.34.0';"), '');
}
