// frenet-tracking-webhook v5 — append-only local + ERP reconciliation ledger
...
const L=(step:string,d:Record<string,unknown>={})=>console.log(JSON.stringify({fn:'frenet-tracking-webhook',v:5,step,...d}));
...
function tokenConfere(req:Request):boolean{
  if(!TOKEN_NAME||!TOKEN_VALUE) return false;
  const recebido=req.headers.get(TOKEN_NAME);
