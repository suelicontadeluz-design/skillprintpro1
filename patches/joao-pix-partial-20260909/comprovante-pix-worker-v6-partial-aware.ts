// comprovante-pix-worker v6-partial-aware — 09/09/2026
// - partial receipts are valid financial evidence when <= current balance
// - receipt identity is file evidence, not amount (two equal installments remain distinct)
// - duplicate bridge events do not notify admin twice
// - order becomes paid/Purchase only when cumulative balance reaches zero
// - preserves safe non-PIX document fallback
...
const VERSION='v6-partial-aware';
...
async function analisarArquivo(fileUrl:string,mimeType:string,isPdf:boolean):Promise<{eh_comprovante:boolean;valor:number|null;confianca:string;file_size:number}>{
  try{
...
{"eh_comprovante": true/false, "valor": 131.13 ou null, "confianca": "alta/media/baixa", "motivo": "texto curto"}
Regras:
- eh_comprovante = true se for comprovante de pagamento Pix, TED, DOC, boleto ou transferencia bancaria
- eh_comprovante = false se for arte grafica, logo, imagem de produto, documento qualquer que nao seja comprovante
- valor = valor total pago (numero decimal sem R$). null se nao conseguir ler
- confianca = alta/media/baixa
Nao inclua nada alem do JSON.`;
    const apiRes=await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','x-api-key':ANTHROPIC_KEY,'anthropic-version':'2023-06-01'},body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:150,messages:[{role:'user',content:[contentBlock,{type:'text',text:prompt}]}]}),signal:AbortSignal.timeout(30000)});
    const data=await apiRes.json();const texto=data?.content?.[0]?.text?.trim()??'{}';L('claude_analise',{texto:texto.slice(0,200)});
    const parsed=JSON.parse(texto.replace(/```json|```/g,'').trim());const valor=parsed.valor?parseFloat(String(parsed.valor).replace(',','.')):null;
    return {eh_comprovante:!!parsed.eh_comprovante,valor:valor&&!isNaN(valor)?valor:null,confianca:parsed.confianca||'baixa',file_size:fileSize};
  }catch(e:any){L('analise_erro',{error:String(e?.message||e)});return {eh_comprovante:false,valor:null,confianca:'baixa',file_size:0}}
}
...
Deno.serve(async(req)=>{
  ...
  const {eh_comprovante,valor:valorPagoRaw,confianca,file_size:fileSize}=await analisarArquivo(image_url,mimeType,isPdf);
  ...
  // If Mercado Pago already approved the same amount very recently, this uploaded receipt is evidence of the same provider payment, not a new payment.
  ...
  if(providerDup){
    L('receipt_matches_provider_payment',{eventKey,payment_id:providerDup.payment_id,valorPago});
    return new Response(JSON.stringify({ok:true,handled:true,confirmado:true,duplicate_provider_payment:true,payment_id:providerDup.payment_id,valorPago,payment_state:stateBefore,worker_version:VERSION}),{status:200,headers:{'content-type':'application/json'}});
  }
  ...
  L('comparacao_saldo',{eventKey,valorPago,valorOrcamento,paidBefore,saldoAntes,aplicavel,resultado,confianca});

  const joaoBridge=await enfileirarJoaoPix({eventKey,leadId:lead_id,phone:phone??null,subscriberId:subscriber_id??null,orcamentoId:orc.id,resultado,valorPago,valorOrcamento,confianca,sourceEvidence:{edge_function:'comprovante-pix-worker/v6-partial-aware',mime_type:mimeType,is_pdf:isPdf,deal_id:orc.deal_id??null,worker_status_before:orc.status,file_fingerprint:fingerprint,paid_before:paidBefore,balance_before:saldoAntes}});
  L('joao_bridge',{eventKey,joaoBridge});

  if(joaoBridge?.status==='DUPLICATE')return new Response(JSON.stringify({ok:true,handled:true,duplicate:true,confirmado:resultado==='CONFIRMADO',valorPago,joao_bridge:joaoBridge,worker_version:VERSION}),{status:200,headers:{'content-type':'application/json'}});

  const persisted=['ENQUEUED','SUPPRESSED'].includes(String(joaoBridge?.status||''));
  if(resultado==='CONFIRMADO'&&!persisted){
    await notificarAdmin(`*Falha ao registrar comprovante Pix*\n\nCliente: ${nome||'Cliente'} (${phone})\nValor identificado: ${brl(valorPago)}\nOrçamento: ${brl(valorOrcamento)}\nSaldo antes: ${brl(saldoAntes)}\nBridge: ${joaoBridge?.status||'desconhecido'}\n\nPagamento NÃO foi aplicado automaticamente.`);
    return new Response(JSON.stringify({ok:false,handled:true,confirmado:false,error:'payment_evidence_not_persisted',joao_bridge:joaoBridge,worker_version:VERSION}),{status:503,headers:{'content-type':'application/json'}});
  }
  ...
  if(quitado){
    ...
    const purchaseEventId=`mp_order_${orc.id}`;
    const legacyId=`purchase_julia_${String(phone??'').replace(/\D/g,'')}_${orc.id}`;
    ...
    if(!existePe){
      await sb.from('pixel_events').insert({lead_id,event_name:'Purchase',event_time:new Date().toISOString(),event_id:purchaseEventId,event_source:'chat',source:leadData?.utm_source??null,medium:leadData?.utm_medium??null,campaign_id:leadData?.utm_campaign_id??null,adset_id:leadData?.utm_adset_id??null,ad_id:leadData?.utm_ad_id??null,content_category:leadData?.content_category??'impressao_dtf_textil',value:valorOrcamento,currency:'BRL'});
      void callInternal('capi-routed',{lead_id,event_name:'Purchase',value:valorOrcamento,event_id:purchaseEventId});
    }
  ...
  return new Response(JSON.stringify({ok:true,handled:true,confirmado:true,parcial:!quitado,quitado,valorPago,valorOrcamento,totalPago,saldo:saldoDepois,confianca,joao_bridge:joaoBridge,payment_state:stateAfter,worker_version:VERSION}),{status:200,headers:{'content-type':'application/json'}});
});
