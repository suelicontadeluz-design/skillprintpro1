...
import "https://raw.githubusercontent.com/suelicontadeluz-design/skillprintpro1/7fad2a5883798d4a673dc47bb8534243e4411f4b/patches/_vendor/f4cde19/patches/joao-harness-prompt-skills-v11/skill-quote-table-v2.ts";

declare const Deno: any;

// Governed canary artifact anchored in Git.
// Derived from the formally audited replay candidate. It intentionally injects
// João's native `_dry_run=true`, so this canary is effect-zero and MUST NOT be
// treated as the production rollout artifact.
...
const CS_VERSION = 'skill_continuity_short/v2-canary';
...
function csFinal(raw:string,arteAnterior:string): any | null { let j:any; try{j=JSON.parse(raw);}catch{return null;} const replayBlockedFinancialAuth=j?.ok===false&&j?.erro==='autorizacao_nao_emitida'&&j?.display_data; if(j?.ok!==true&&!replayBlockedFinancialAuth)return null; const d=j.display_data; if(!d)return null; const cop=Number(d?.copias), metros=Number(d?.metros), pm=Number(d?.preco_por_metro), total=Number(d?.valor_total); if(!Number.isFinite(cop)||!Number.isFinite(metros)||!Number.isFinite(pm)||!Number.isFinite(total))return null; const arteCalc=String(d?.arte||'').trim(); const arteBase=String(arteAnterior||'').trim(); const arte=arteBase&&arteCalc&&!arteBase.includes(arteCalc)?`${arteBase} ${arteCalc}`:(arteCalc||arteBase); let mensagem=`Arte ${arteCalc||arte}: ${cop} cópias usam ${String(metros).replace('.',',')}m a ${csMoney(pm)}/m = *${csMoney(total)}*.`; if(d?.cobrado_minimo===true&&Number(d?.copias_sem_aumentar_valor)>cop){mensagem=`Arte ${arteCalc||arte}: ${cop} cópias ficam no mínimo de ${String(d?.minimo_metros??metros).replace('.',',')}m, total *${csMoney(total)}*. Nesse mesmo valor cabem até ${Number(d.copias_sem_aumentar_valor)} cópias.`;} mensagem += ' Se for entrega, me passa o CEP; se preferir retirada, me avisa.'; return {responde:true,mensagem,tema:'dtf_metro',encaminhou_venda:false,etapa:'orcamento',slots:{produto:'dtf_textil',arte,quantidade:cop}}; }
...
(Deno as any).serve=(...args:any[])=>{
  ...
  args[handlerIndex]=async(req:Request,info:any)=>{
    let phone=''; let forwarded=req;
    try{
      if(req.method==='POST'){
        const b=await req.clone().json().catch(()=>null);
        phone=String(b?.phone??b?.telefone??'').replace(/\D/g,'');
        if(b&&typeof b==='object'){
          const headers=new Headers(req.headers); headers.delete('content-length'); headers.set('content-type','application/json');
          forwarded=new Request(req.url,{method:req.method,headers,body:JSON.stringify({...b,_dry_run:true})});
        }
      }
    }catch{}
    return csAls.run({phone},()=>handler(forwarded,info));
  };
...
globalThis.fetch=async(input:RequestInfo|URL,init?:RequestInit):Promise<Response>=>{
  ...
  const own=csOwnResult(body.messages);
  if(own){const d=csFinal(own.raw,own.arteAnterior); if(!d){await csMetric('tool_result_unusable');return CS_BASE_FETCH(input,init);} await csMetric('final_deterministic'); return csAnthropicText(d);}

  const inbound=csInbound(body.messages); const n=csShortInt(inbound);
  if(!n)return CS_BASE_FETCH(input,init);
  await csMetric('bare_number_seen');

  const slots=csSlots(body.system);
  if(String(slots?.produto||'').toLowerCase()!=='dtf_textil'||Number(slots?.quantidade)>0){await csMetric('fail_product_or_quantity');return CS_BASE_FETCH(input,init);}

  const phone=csAls.getStore()?.phone||'';
  if(!phone){await csMetric('fail_phone_missing');return CS_BASE_FETCH(input,init);}

  const hist=await csHistory(phone);
  if(!hist){await csMetric('fail_history');return CS_BASE_FETCH(input,init);}
  ...
  const dim=csDimension(hist.inbounds);
  if(!dim){await csMetric(`no_recent_dimension_rows_${hist.inbounds.length}`);return CS_BASE_FETCH(input,init);}

  await csMetric('tool_request_live_canary');
  return csAnthropicTool(dim.largura,dim.altura,n,String(slots?.arte||''));
};
