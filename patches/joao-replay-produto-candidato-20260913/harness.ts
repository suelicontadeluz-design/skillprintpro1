...
function bloquear(raw: string, metodo: string, motivo: string, alvo?: string, input?: any, init?: any) {
  const s = store();
  if (s) {
...
async function baseFetch(input: any, init?: any): Promise<Response> {
  ...
  return bloquear(raw, metodo, motivo, alvo, input, init);
}
  ...
  Object.defineProperty(globalThis, 'fetch', {
    ...
    set(fn: any) {
      ...
      current = async (input: any, init?: any) => {
        ...
        if (d.decisao === 'BLOCK') return bloquear(raw, metodo, d.motivo, d.alvo, input, init);
