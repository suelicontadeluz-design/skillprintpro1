...
const V="order-email-brevo-dispatcher/v5";
...
function render(event:any){
  ...
  if(eventType==="message.sale.confirmed"){
    const items=Array.isArray(p.items)?p.items:[];
    const itemRows=items.map((x:any)=>`
      <tr>
        <td style="padding:11px 0;border-bottom:1px solid #f1e5e9;color:#3f3337;font-size:14px">
          <strong style="color:#26171c">${esc(x?.name??"Item")}</strong><br>
          <span style="color:#8a777d">${esc(qty(x?.quantity))} ${esc(String(x?.unit??"un").replaceAll("_"," "))} × ${esc(money(x?.unit_price??0))}</span>
        </td>
        <td style="padding:11px 0;border-bottom:1px solid #f1e5e9;color:#26171c;font-size:14px;font-weight:700;text-align:right;white-space:nowrap">
          ${esc(money(x?.total??0))}
        </td>
      </tr>`
    ).join("");
    ...
    const shipping=String(p.shipping_type??"").trim()==="retirada"?"Retirada":"Entrega";
    const delivery=dateBR(p.delivery_date);
    subject=`Pedido N°${numero} confirmado`;
    title="Pedido confirmado";
    body=`
      Olá, <strong>${esc(nome)}</strong>! Recebemos e confirmamos o seu pedido <strong>N°${esc(numero)}</strong>.<br><br>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse">
        ${itemRows}
        <tr>
          <td style="padding:13px 0 5px;color:#7a6b70;font-size:14px">Subtotal</td>
          <td style="padding:13px 0 5px;text-align:right;color:#3f3337;font-size:14px">${esc(money(p.subtotal??0))}</td>
        </tr>
        ${Number(p.freight??0)>0?`<tr><td style="padding:5px 0;color:#7a6b70;font-size:14px">Frete</td><td style="padding:5px 0;text-align:right;color:#3f3337;font-size:14px">${esc(money(p.freight))}</td></tr>`:""}
        ${Number(p.discount??0)>0?`<tr><td style="padding:5px 0;color:#7a6b70;font-size:14px">Desconto</td><td style="padding:5px 0;text-align:right;color:#9A0525;font-size:14px">− ${esc(money(p.discount))}</td></tr>`:""}
        <tr>
          <td style="padding:11px 0 4px;color:#26171c;font-size:16px;font-weight:700">Total</td>
          <td style="padding:11px 0 4px;text-align:right;color:#9A0525;font-size:18px;font-weight:800">${esc(money(p.total??0))}</td>
        </tr>
      </table>
      <div style="margin-top:20px;padding:15px 16px;background:#faf7f8;border:1px solid #efe0e5;border-radius:10px;font-size:14px;line-height:1.65;color:#55484c">
        ${payment?`<div><strong>Pagamento:</strong> ${esc(payment)}</div>`:""}
        <div><strong>Entrega:</strong> ${esc(shipping)}</div>
        ${delivery?`<div><strong>Previsão:</strong> ${esc(delivery)}</div>`:""}
      </div>
      <br>Você receberá novas atualizações por e-mail conforme o pedido avançar.
    `;
  } else if(eventType==="message.order_ready.payment_due"){
    ...
    subject=`Etiqueta gerada — Pedido N°${numero}`;
    title="Etiqueta de envio gerada";
    body=`Olá, ${esc(nome)}! A etiqueta do pedido <strong>N°${esc(numero)}</strong> foi gerada.`
