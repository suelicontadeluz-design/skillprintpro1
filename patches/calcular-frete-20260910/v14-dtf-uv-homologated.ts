...
// calcular-frete v14 — 10/09/2026
// Mantem integralmente as guardas do v13 e adiciona DTF UV com perfil fisico homologado pela operacao.
// DTF UV: caixa 30x13x13 cm, 1 kg, faixa homologada ate 10 m.
// Acima de 10 m: fail-closed ate nova afericao. O consumo UV vem do recibo canonico do ERP (uv_consumo_m).
...
const DTF_UV_PRODUTO_ID = 'd48addf9-2b53-482f-8f28-1dfc7ea7c123';
const DTF_UV_CAIXA = { comprimento: 30, largura: 13, altura: 13 };
const DTF_UV_PESO_KG = 1;
const DTF_UV_MAX_HOMOLOGADO_M = 10;
...
function itemEhDtfUv(item: any): boolean {
  return String(item?.produto_id || '') === DTF_UV_PRODUTO_ID || /dtf\s*uv/.test(norm(item?.descricao || ''));
}
function consumoUvCanonico(items: any[]): number | null {
  let total = 0;
  for (const item of items) {
    const raw = item?.uv_consumo_m;
...
Deno.serve(async (req) => {
  ...
  const allDtfUv = items.every((i: any) => itemEhDtfUv(i));
  ...
  if (allGarments) {
    ...
    const metrosUv = consumoUvCanonico(items);
    if (!metrosUv || metrosUv <= 0) {
      return json({
        ok: false,
        error: 'dtf_uv_consumo_canonico_ausente',
        acao: 'Nao calcule frete UV sem uv_consumo_m no recibo canonico do ERP.'
      }, 409);
    }
    if (metrosUv > DTF_UV_MAX_HOMOLOGADO_M) {
      return json({
        ok: false,
        error: 'dtf_uv_cubagem_fora_da_faixa',
        quantidade_m: metrosUv,
        faixa_homologada: 'ate 10 m',
        acao: 'Acima de 10 m exige nova afericao de peso/volume antes de cotar.'
      }, 409);
    }
    emb = { ...DTF_UV_CAIXA, pesoKg: DTF_UV_PESO_KG, pesoG: 1000 };
    shipmentKind = 'dtf_uv';
    quantidadeLogistica = metrosUv;
    unidadeLogistica = 'metros';
    sku = 'DTF-UV';
    category = 'DTF UV';
    regraPeso = '1kg_caixa_30x13x13_ate_10m_operacao_skillprint_20260910';
    cubagemEstimada = false;
    metrosSnapshot = metrosUv;
  } else {
    return json({
      ok: false,
  ...
  const sourceEvidence = {
    ...
    edge_function: 'calcular-frete/v14',
