...
// João Product/Service Disambiguation v3 — 10/09/2026
// Mudança mínima sobre v2: reconhece intenção explícita de aplicar/estampar em pano/tecido.
// Evita transformar menção secundária a caneca em DTF UV quando o cliente já disse que quer tecido.
// Preserva os casos v2: peça pronta explícita não pergunta; "caneca e camisa" continua ambíguo.
// Não cria heurística geral de negação/tempo para manter blast radius pequeno.
...
const PD_VERSION = 'joao-product-service-disambiguation/v3-textile-intent';
let pdCfgAt = 0;
let pdCfg = false;
...
function pdExplicitMode(inbound: string): boolean {
  ...
  const textileSubstrateExplicit = /\b(?:passar|aplicar|estampar|imprimir)\b.{0,40}\b(?:em|no|na|nos|nas|para|pra)\s+(?:o\s+)?(?:tecido|pano(?: de prato)?)\b/.test(t)
    || /\b(?:quero|preciso)\b.{0,35}\b(?:para|pra)\s+(?:o\s+)?(?:tecido|pano(?: de prato)?)\b/.test(t);
  const transferExplicit = /\bdtf\b/.test(t)
    || /\b(?:transfer(?:encia)?|adesivos?|folhas?|metros?)\b/.test(t)
    || /\b(?:so|somente|apenas)\s+(?:a\s+)?(?:impressao|estampa|transfer|adesivo)\b/.test(t)
    || textileSubstrateExplicit;

  const piece = '(?:produto|peca|caneca|copo|camisa|camiseta|baby\\s*look|oversized|moletom|polo)';
  const finishedExplicit = new RegExp(`\\b${piece}\\b.{0,40}\\b(?:pront[oa]s?|personalizad[oa]s?)\\b`).test(t)
    || new RegExp(`\\b(?:pront[oa]s?|personalizad[oa]s?)\\b.{0,40}\\b${piece}\\b`).test(t)
    || new RegExp(`\\b(?:ja\\s+)?(?:no|na|nos|nas)\\s+(?:copo|caneca|camisa|camiseta|baby\\s*look|oversized|moletom|polo)\\b`).test(t)
    || new RegExp(`\\b(?:aplicad[oa]s?|estampad[oa]s?)\\b.{0,30}\\b(?:no|na|nos|nas)\\s+(?:copo|caneca|camisa|camiseta|baby\\s*look|oversized|moletom|polo)\\b`).test(t)
    || /\b(?:quero|preciso)\b.{0,35}\b(?:pront[oa]s?|personalizad[oa]s?)\b/.test(t);

  return transferExplicit || finishedExplicit;
}
