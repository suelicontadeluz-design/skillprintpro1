# -*- coding: utf-8 -*-
"""v4.38.0 P0 FASE 2 - fato comercial sem lastro tambem nao pode ser DITO.

Base: v4.37.0 (porta de escrita), publicada e aprovada no canario da fase 1.
Esta frente NAO toca na porta de escrita: so acrescenta a guarda de SAIDA.
"""
...
assert hashlib.sha256(src.encode('utf-8')).hexdigest() == \
    '58ecc6b53aa9bca59ccf7d0398fba9c07d2488f75b3c4818bd72bd7f7b574816', 'base nao e o candidato v4.37.0 da fase 1'

trocas = []
def rep(a, n, r): trocas.append((a, n, r))

rep("const V = 'agente-noturno-v4.37.0';",
"""//
// v4.38.0 (27/08/2026) P0 FASE 2 - O FATO SEM LASTRO TAMBEM NAO PODE SER DITO.
// Rollback: redeploy da v4.37.0. Sem migracao, sem estado novo.
//
// A porta de escrita da v4.37.0 impede o fato de virar ESTADO. Mas decisao.mensagem
// nasce ANTES dela: o modelo ainda podia FALAR "os 300 adesivos" com os slots ja
// recusados. Entra uma guarda de SAIDA no INVARIANTE DE TRANSPORTE, depois de todas
// as reescritas e antes da guarda financeira.
//
// A fonte de verdade e slotsNovos - o snapshot que a porta ja aprovou. decisao.slots
// CRU nao entra. Sem lastro: retry explicito -> revalida -> poda cirurgica (a mesma
// removerSentencasComTermo da v4.34.0) -> texto neutro. Nunca silencio.
//
// DUAS regras, estreitadas contra 284 turnos organicos (a versao ampla bloqueava
// 47,7% do trafego normal e nao era publicavel):
//   quantidade_veio_de_dinheiro - o numero afirmado nasceu GRUDADO num marcador de
//     dinheiro na fala do cliente. Adjacencia, nao a frase inteira.
//   produto_contradiz_pedido    - o texto nomeia familia que o pedido nao admite e
//     que o cliente nunca nomeou. Compara com um CONJUNTO de familias.
// Medicao final: 2 disparos em 281 turnos (0,7%), zero falso positivo, ambos reais.
const V = 'agente-noturno-v4.38.0';""", "cabecalho + const V")

MODULO_SAIDA = """// ── GUARDA DE SAIDA: fato comercial AFIRMADO ao cliente ────────────────────
...
rep("""// A PORTA. Devolve os slots que podem virar fato + a lista do que foi recusado.""",
    MODULO_SAIDA + """// A PORTA. Devolve os slots que podem virar fato + a lista do que foi recusado.""",
    "modulo da guarda de saida")
...
"""  // ── v4.37.0 P0: FATO COMERCIAL AFIRMADO SEM LASTRO ────────────────────
  ...
  if (decisao.responde === true) {
    // numerosFerramenta ja foi montado pela porta de escrita (v4.37.0) com rendimentos
    // + itens do CalcMe. Reusa, nao remonta.
    const numerosAut: number[] = numerosFerramenta.slice();
...
for antigo, novo, nome in trocas:
    n = src.count(antigo)
    assert n == 1, 'ancora "' + nome + '" apareceu ' + str(n) + ' vezes (esperado 1)'
    src = src.replace(antigo, novo)

io.open(OUT, 'w', encoding='utf-8').write(src)
sys.stderr.write('candidato v4.38.0 escrito: ' + str(len(src)) + ' bytes, sha256 '
