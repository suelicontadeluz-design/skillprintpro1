...
// João Qualification Production Gate v1.6 — 10/09/2026
// v1.6: HOLD de CEP responde sem reenviar slots antigos; preserva proveniência fail-closed.
...
const QG_VERSION = 'joao-qualification-gate/v1.6';
let qgCfgAt = 0;
let qgCfg = false;
...
function qgDecision(status: string, slots: any): any | null {
  ...
  if (status === 'HOLD_MISSING_CEP_FOR_SHIPPING') return { responde: true, mensagem: 'Me passa o CEP de entrega que eu calculo as opções de frete.', tema: 'frete', encaminhou_venda: false, etapa: 'orcamento' };
  return null;
}
