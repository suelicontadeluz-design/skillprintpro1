...
// PricingAgent Phase 1 — explicit DTF UV sheet quote gate v3 — 15/09/2026
...
const P3_BASE_SERVE=Deno.serve.bind(Deno);
const P3_BASE_FETCH=globalThis.fetch.bind(globalThis);
const P3_ERP_URL=(Deno.env.get('ERP_URL')??'https://ynjsflvdfftcopibzxyo.supabase.co').replace(/\/$/,'');
const P3_ERP_KEY=Deno.env.get('ERP_SERVICE_KEY')??Deno.env.get('ERP_SERVICE_ROLE_KEY')??'';
const P3_VERSION='pricing-agent-explicit-sheet/v3';
...
function uvResolved(incoming:string,payload:any){const t=norm(incoming),p=norm(payload?.slots?.produto??payload?.produto??'');return /\b(adesivo|adesivos|dtf\s*uv|uv)\b/.test(t)||['dtf_uv','dtf uv','adesivo_uv','adesivo uv','adesivo_dtf_uv','adesivo dtf uv'].includes(p)}
...
(Deno as any).serve=(...args:any[])=>{
  ...
  args[hi]=async(req:Request,info:any)=>{
    ...
    const incoming=String(body?.mensagem??body?.message??'').trim(),order=parseSheet(incoming);if(!order)return res;
    let raw='';try{raw=await res.clone().text()}catch{return res}if(!raw.trim().startsWith('{'))return res;
    let payload:any;try{payload=JSON.parse(raw.trim())}catch{return res}if(!payload||typeof payload!=='object'||!uvResolved(incoming,payload))return res;

    const q=await quote(order),mixed=mixedSensitive(incoming);
    const headers=new Headers(res.headers);headers.delete('content-length');headers.set('content-type','application/json; charset=utf-8');headers.set('x-cortex-pricing-agent',P3_VERSION);
    const meta={gate:'ERP_CANONICAL_EXPLICIT_SHEET_QUOTE',current_turn:true,format:order.format,count:order.count,dry_run:body?._dry_run===true,source:'ERP',version:P3_VERSION,mixed_sensitive:mixed};

    // Mixed turns are never rewritten in Phase 1. Quote is observation/evidence only.
    ...
    console.log(JSON.stringify({event:'PRICING_AGENT_EXPLICIT_SHEET_ENFORCED',version:P3_VERSION,format:order.format,count:order.count,unit_price:unit,total_price:total,composition,dry_run:body?._dry_run===true}));
    return new Response(JSON.stringify(next),{status:res.status,statusText:res.statusText,headers});
  };
  return P3_BASE_SERVE(...args as any);
};
