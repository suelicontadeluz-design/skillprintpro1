-- Patch v1.1: require resolved ERP client and only rebind proposal from Cliente Balcao.
...
declare
  ...
  v_current_pessoa_nome text;
  v_client_action text:='unchanged';
begin
  ...
  if coalesce(p_proof->>'cliente_id','') !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then return jsonb_build_object('ok',false,'code','ERP_CLIENT_ID_REQUIRED'); end if;
  v_cliente_id := (p_proof->>'cliente_id')::uuid;
  select exists(select 1 from public.pessoa_cliente_dados where id=v_cliente_id) into v_cliente_exists;
  if not v_cliente_exists then return jsonb_build_object('ok',false,'code','ERP_CLIENT_NOT_FOUND','cliente_id',v_cliente_id); end if;
  ...
  if v_prop.cliente_id is distinct from v_cliente_id then
    select pe.nome into v_current_pessoa_nome
    from public.pessoa_cliente_dados pcd join public.pessoas pe on pe.id=pcd.pessoa_id
    where pcd.id=v_prop.cliente_id;
    if lower(btrim(coalesce(v_current_pessoa_nome,'')))='cliente balcão' or lower(btrim(coalesce(v_current_pessoa_nome,'')))='cliente balcao' then
      v_client_action:='rebind_from_balcao';
    else
      return jsonb_build_object('ok',false,'code','PROPOSAL_CLIENT_CONFLICT','current_cliente_id',v_prop.cliente_id,'current_name',v_current_pessoa_nome,'resolved_cliente_id',v_cliente_id);
    end if;
  end if;
  ...
  if found then
    if v_sale.proposta_id is distinct from v_prop.id or abs(coalesce(v_sale.total_com_frete,v_sale.total_liquido,0)-p_payment_total)>0.01 then return jsonb_build_object('ok',false,'code','IDEMPOTENCY_CONFLICT','venda_id',v_sale.id); end if;
  ...
  select count(*),count(*) filter(where pi.produto_id is null or pi.origem_custo<>'motor' or coalesce(pi.contexto_execucao->>'canonical_item','false')<>'true') into v_item_count,v_bad_items from public.proposta_itens pi where pi.proposta_id=v_prop.id;
  ...
  select count(*) into v_bank_n from public.financeiro_contas_bancarias b where b.ativa=true and ((v_bank_key='mercado pago' and (lower(btrim(coalesce(b.nome,'')))='mercado pago' or lower(btrim(coalesce(b.banco,'')))='mercado pago')) or (v_bank_key='itau' and (translate(lower(coalesce(b.nome,'')),'áàâãéêíóôõúç','aaaaeeiooouc') like '%itau%' or translate(lower(coalesce(b.banco,'')),'áàâãéêíóôõúç','aaaaeeiooouc') like '%itau%')) or (v_bank_key='bradesco' and (lower(coalesce(b.nome,'')) like '%bradesco%' or lower(coalesce(b.banco,'')) like '%bradesco%')) or (v_bank_key='c6 bank' and (lower(coalesce(b.nome,'')) like '%c6%' or lower(coalesce(b.banco,'')) like '%c6%')));
  ...
  select b.id into v_bank_id from public.financeiro_contas_bancarias b where b.ativa=true and ((v_bank_key='mercado pago' and (lower(btrim(coalesce(b.nome,'')))='mercado pago' or lower(btrim(coalesce(b.banco,'')))='mercado pago')) or (v_bank_key='itau' and (translate(lower(coalesce(b.nome,'')),'áàâãéêíóôõúç','aaaaeeiooouc') like '%itau%' or translate(lower(coalesce(b.banco,'')),'áàâãéêíóôõúç','aaaaeeiooouc') like '%itau%')) or (v_bank_key='bradesco' and (lower(coalesce(b.nome,'')) like '%bradesco%' or lower(coalesce(b.banco,'')) like '%bradesco%')) or (v_bank_key='c6 bank' and (lower(coalesce(b.nome,'')) like '%c6%' or lower(coalesce(b.banco,'')) like '%c6%'))) limit 1;
  ...
  if v_dry then return jsonb_build_object('ok',true,'code','MANUAL_SETTLEMENT_READY_DRY_RUN','proposta_id',v_prop.id,'numero_proposta',v_prop.numero_proposta,'total',v_expected,'bank_key',v_bank_key,'bank_id',v_bank_id,'item_count',v_item_count,'cliente_id',v_cliente_id,'client_action',v_client_action,'dry_run',true); end if;
  if v_client_action='rebind_from_balcao' then update public.propostas set cliente_id=v_cliente_id,updated_at=now() where id=v_prop.id; v_prop.cliente_id:=v_cliente_id; end if;
