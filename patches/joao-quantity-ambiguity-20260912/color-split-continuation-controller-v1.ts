...
// João color-split continuation controller v1.2 — 12/09/2026
...
// já informado, força calcular_frete. Depois do tool_result de frete, preserva a linguagem do modelo,
// mas corrige deterministicamente quantidade/CEP e garante que o preço canônico do produto não suma.

const QCC_VERSION='joao-color-split-continuation/v1.2';
...
async function patchFinalModelResponse(response:Response,res:{mode:'each'|'single',n:number},colors:string[],cep:string,product:any):Promise<Response>{
  ...
  let changed=false;
  body.content=body.content.map((b:any)=>{
    if(b?.type!=='text'||typeof b.text!=='string')return b;let d:any;try{d=JSON.parse(b.text);}catch{return b;}if(!d||typeof d!=='object')return b;
    d.slots={...(d.slots||{}),produto:'DTF UV',quantidade:qtdDesc,cep,envio_retirada:'envio',modalidade_logistica:'envio'};
    d.tema='frete';d.etapa='fechamento';d.encaminhou_venda=true;
    const msg=String(d.mensagem||'');
    if(amount&& !amountMentioned(msg,amount)) d.mensagem=`Fechando em ${qtdTotal} adesivos: DTF UV R$ ${amount.toFixed(2).replace('.',',')}. ${msg}`.trim();
    changed=true;return {...b,text:JSON.stringify(d)};
  });
...
globalThis.fetch=async(input:RequestInfo|URL,init?:RequestInit):Promise<Response>=>{
  ...
  const latestFreight=freightUses.length?freightUses[freightUses.length-1]:null;const freightRaw=latestFreight?toolResultRaw(req.messages,latestFreight.id,latestFreight.index):null;const freight=normalizeResult(freightRaw);const freightOk=!!latestFreight&&resultOk(freightRaw,freight);
  ...
  if(productOk&&freightOk){
    const modelResponse=await qccBaseFetch(input,init);
    return await patchFinalModelResponse(modelResponse,res,colors,cep,product);
  }
